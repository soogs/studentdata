...
class DataTransformation:
	...
	def initiate_data_transformation(self, train_path, test_path):
		...
		try:
			...
			# the df for predictors that excludes the target feature
			input_feature_train_df = train_df.drop(columns = [target_column_name], axis = 1)

			# the target feature in the train set
			target_feature_train_df=train_df[target_column_name]


			# df for predictors that exclude the target feature
			input_feature_test_df=test_df.drop(columns=[target_column_name],axis=1)

			# target feature in the test set
			...
			# concatenate the predictor and target featues
			train_arr = np.c_[
